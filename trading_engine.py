...
def normalize_signal(payload: dict[str, Any]) -> Signal:
    """Temporary generic mapping; replace after a real payload is captured."""
    ...
    if not sid:
        raise ValueError("missing signal_id/id")
    if not symbol:
        raise ValueError("missing symbol/scrip")
    if side != "BUY":
        raise ValueError("buy-only safety policy blocked non-BUY signal")
    if qty is not None:
        qty = int(qty)
        if qty <= 0:
